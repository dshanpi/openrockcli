...
int openrockcli_scan(libusb_context * context)
{
	...
	for(ssize_t i = 0; i < count; i++)
	{
		...
		printf("%03u  ", libusb_get_bus_number(device));
		if(nports > 0)
		{
			for(int p = 0; p < nports; p++)
				printf("%s%u", p ? "." : "", ports[p]);
			printf("%*s", (nports < 4) ? (int)(6 - (nports * 2)) : 2, "");
		}
		else
			printf("-     ");
		...
		printf(" %04x:%04x  %-8s  %s\r\n",
			...
			openrockcli_chip_name(desc.idProduct));
		found++;
	}
...
static inline void rock_maskrom_write_arm32(struct openrockcli_ctx_t * ctx, uint32_t addr, void * buf, size_t len, int rc4)
{
...
static inline void rock_maskrom_write_arm64(struct openrockcli_ctx_t * ctx, uint32_t addr, void * buf, size_t len, int rc4)
{
	static const uint8_t payload[] = {
...
static inline int rock_read_raw(struct openrockcli_ctx_t * ctx, uint32_t addr, void * buf, size_t len)
{
	struct usb_request_t req;
	struct usb_response_t res;
...
static inline int rock_write_raw(struct openrockcli_ctx_t * ctx, uint32_t addr, void * buf, size_t len)
{
	struct usb_request_t req;
	struct usb_response_t res;
...
static inline int rock_flash_erase_lba_raw(struct openrockcli_ctx_t * ctx, uint32_t sec, uint32_t cnt)
{
	struct usb_request_t req;
	struct usb_response_t res;
...
static inline int rock_flash_read_lba_raw(struct openrockcli_ctx_t * ctx, uint32_t sec, uint32_t cnt, void * buf)
{
	struct usb_request_t req;
	struct usb_response_t res;
...
static inline int rock_flash_write_lba_raw(struct openrockcli_ctx_t * ctx, uint32_t sec, uint32_t cnt, void * buf)
{
	struct usb_request_t req;
	struct usb_response_t res;
